...
Congrats 🥳! You have trained your first gradient boosting model. Now, we will try to find the best hyperparameters to minimize the final predicted error of the model.

::::

:::::: {.callout-tip collapse="false" icon="false"}
## <i class="bi bi-book"></i> Exercice 5: Tuning gradient boosting's hyperparameters

Default values are usually a good start but we need to tune its hyperparameters to find out if a better model doesn't exist. In this exercise, your aim is to train the best model for the prediction of price per square meter.
